...
class LeggedEnvZ:
    def __init__(self, num_envs, env_cfg, obs_cfg, noise_cfg, reward_cfg, command_cfg, terrain_cfg, show_viewer_=False, eval_=False, show_camera_=False, control_=False, device="cuda"):
        ...
        if self.env_cfg["use_mjcf"]:
            self.robot  = self.scene.add_entity(
                gs.morphs.MJCF(
                # file="/home/techshare/Zxxx_tt/go2Zoid/go2Zxxx.xml", # plz, path to Z model.
                # file="/home/techshare/drl_kit/mujoco_ws/ts_mujoco/unitree_robots/go2Zoid/go2Zxxx.xml", # plz, path to Z model.
                file="/home/techshare/drl_kit/mujoco_ws/ts_mujoco/unitree_robots/go2Zoid/go2Zxxx_collision.xml", # plz, path to Z model.
    ...
    def step(self, actions):
        ...
        ### used reward step distance, step timing
    ...
    def _reward_tracking_lin_vel(self):
        ...
        # print("lin_vel_error: ", lin_vel_error.shape)
    ...
    def _reward_tracking_ang_vel(self):
        ...
        # print("ang_vel_error: ", ang_vel_error.shape)
    ...
    def _reward_orientation_x(self):
        # Penalize non flat base orientation x-coodinates
    ...
    def _reward_ang_vel_x(self):
        ...
        # print("ang_vel_x: ", torch.square(self.base_ang_vel[:, 0]).shape)
    ...
    def _reward_stand_still(self):
        ...
        # return torch.sum(torch.abs(self.dof_pos - self.default_dof_pos), dim=1) * (torch.norm(self.commands, dim=1) < 0.05)
        penalty = torch.sum(torch.abs(self.dof_pos - self.default_dof_pos), dim=1) * (torch.norm(self.commands, dim=1) < 0.1)
        penalty += torch.sum(torch.abs(self.dof_vel), dim=1) * (torch.norm(self.commands, dim=1) < 0.1)
        return penalty
    ...
    def _reward_step_timing(self):
        ...
        gait_timing = torch.square(self.elapsed_air_time - self.step_period/2)
    ...
    def _reward_swing_foot(self):
        # 要改良.
        swing = (self.leg_phase >= 0.02) & (self.leg_phase < 0.48)
        contact = torch.norm(self.contact_forces[:, self.feet_indices, :], dim=2) > 1.
                
        penalty = torch.sum((contact ^ swing) & (torch.norm(self.commands, dim=1) < 0.05).unsqueeze(1),dim=1)
