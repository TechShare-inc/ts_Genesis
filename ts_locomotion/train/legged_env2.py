...
class LeggedEnv2:
    def __init__(self, num_envs, env_cfg, obs_cfg, noise_cfg, reward_cfg, command_cfg, terrain_cfg, show_viewer_=False, eval_=False, show_camera_=False, control_=False, device="cuda"):
        ...
        self.curriculum = command_cfg["curriculum"]
        self.curriculum_tasks = command_cfg["curriculum_tasks"]
        self.curriculum_level_flag = command_cfg["start_curriculum"]
        ...
        self.sim_substeps = 1
        self.mean_reward = 0 # this computed in on_policy_runner.py
        ...
        self.history_length = max(int(env_cfg['control_freq']), 2)
        ...
        # visualized_number = min(self.num_envs, 100)        # capped at 100
        visualized_number = min(self.num_envs, 1)        # capped at 1
        ...
        if self.terrain_type != "plane" and self.terrain_type != "custom_plane":
            if self.terrain_type == "variable_terrain":
                n_subterrains = (1, 1)

                terrain_types = [["variable_terrain2"]]

                self.vertical_scale = .005
                self.horizontal_scale = .01 # 1/100
                ...
                #     subterrain_size=(total_width, total_height),
                #     n_subterrains=n_subterrains,
                #     horizontal_scale=self.horizontal_scale,
                #     vertical_scale=self.vertical_scale,
                #     subterrain_types=terrain_types,
                # )
                # self.terrain.material = gs.materials.Rigid(friction=0.8)
                
                self.terrain = gs.morphs.URDF(file="urdf/plane/variable_terrain.urdf",
                                              pos=(-self.center_x, -self.center_y, 0),
                                              scale=1.0,
                                              fixed=True),
                self.scene.add_entity(
                    gs.morphs.URDF(file="urdf/plane/variable_terrain.urdf", 
                                   pos=(-self.center_x, -self.center_y, 0),
                                   scale=1.0,
                                   fixed=True),
                )
                ...
                each_width = int(total_width/self.cols/self.horizontal_scale)
                each_half_width = int(each_width/2)
                each_height = int(total_height/self.rows/self.horizontal_scale)
                each_half_height = int(each_height/2)

                stair_step_height_map = int(0.3/self.horizontal_scale) # 0.3 = a step of stair
                go2_scale = int(0.8/self.horizontal_scale) # go2's xy-length(L2-norm)

                height = np.zeros((int(total_width/self.horizontal_scale), int(total_height/self.horizontal_scale)), dtype=np.float32)

                # plane
                height[:each_width+go2_scale] = 1.8
                height[2*each_width:2*each_width+go2_scale] = .9
                height[3*each_width:3*each_width+go2_scale] = .3

                # stair
                for i in range(6): # 0.3[m] * 6 = 1.8[m]
                    height[2*each_width-(i+1)*stair_step_height_map:2*each_width-i*stair_step_height_map, each_height:2*each_height] = 1.05 + .15 * i # 1.05 = 0.9 + 0.15
                    height[3*each_width-(i+1)*stair_step_height_map:3*each_width-i*stair_step_height_map, each_height:2*each_height] = .4 + .1 * i # 0.4 = 0.3 + 0.1
                    height[4*each_width-(i+1)*stair_step_height_map:4*each_width-i*stair_step_height_map, each_height:2*each_height] = .05 * (i+1)
                
                # boundary
                for i in range(each_width):
                    height[2*each_width-i-1, [0, -1]] = .9 * (1 + (i+1)/each_width)
                    height[3*each_width-i-1, [0, -1]] = .3 + .6 * (i+1)/each_width
                    height[4*each_width-i-1, [0, -1]] = .3*(i+1)/each_width

                # slope
                for j in range(1, each_height):
                    height[each_width:2*each_width, j] = (height[each_width:2*each_width, 0]*(each_height-j) + height[each_width:2*each_width, each_height]*j)/each_height
                    height[2*each_width:3*each_width, j] = (height[2*each_width:3*each_width, 0]*(each_height-j) + height[2*each_width:3*each_width, each_height]*j)/each_height
                    height[3*each_width:4*each_width, j] = (height[3*each_width:4*each_width, 0]*(each_height-j) + height[3*each_width:4*each_width, each_height]*j)/each_height

                height[:, -each_height:] = np.flip(height[:, :each_height], axis=1)
                height /= self.vertical_scale

                self.height_field = height
                
                print(f"terrain: {each_width}*{each_height}")

                go2_scale = int(0.8/self.horizontal_scale) # 0.8 = go2's xy-length(L2-norm)
                half_go2_scale = int(go2_scale/2)

                # plane, center plane
                self.spawn_points = {"plane": (5*each_width, each_half_height), 
                                     "plane_center": (5*each_width + each_half_width, 3*each_half_height),
                                     "rough": (each_half_width, each_half_height), 
                                     "rough_center": (each_half_width, each_half_height),
                                     "stair_5cm_center": (4*each_width + each_half_width, 3*each_half_height),
                                     "stair_10cm_center": (3*each_width + each_half_width, 3*each_half_height),
                                     "stair_15cm_center": (2*each_width + each_half_width, 3*each_half_height),
                                     "slope_0-5cm": (4*each_width + half_go2_scale, each_half_height),
                                     "slope_5-10cm": (3*each_width + half_go2_scale, each_half_height),
                                     "slope_10-15cm": (2*each_width + half_go2_scale, each_half_height),
                                     "slope_15-cm": (each_width + half_go2_scale, each_half_height),
                                     "stair_0-5cm": (4*each_width + half_go2_scale, 3*each_half_height),
                                     "stair_5-10cm": (3*each_width + half_go2_scale, 3*each_half_height),
                                     "stair_10-15cm": (2*each_width + half_go2_scale, 3*each_half_height),
                                     "stair_15-cm": (each_width + half_go2_scale, 3*each_half_height),
                                     }
                # self.probability_spawn_points = {}
            elif self.terrain_type == "variable_terrain3":
                n_subterrains = (1, 1)

                terrain_types = [["variable_terrain3"]]

                self.vertical_scale = .0025
                self.horizontal_scale = .02 # 1/100
                ...
                self.terrain = gs.morphs.URDF(file="urdf/plane/variable3_terrain.urdf",
                                              pos=(-self.center_x, -self.center_y, 0),
                                              scale=1.0,
                                              fixed=True),
                self.scene.add_entity(
                    gs.morphs.URDF(file="urdf/plane/variable3_terrain.urdf", 
                                   pos=(-self.center_x, -self.center_y, 0),
                                   scale=1.0,
                                   fixed=True),
                )
                ...
                # plane
                height[:each_width+go2_scale] = 4.2
                height[2*each_width:2*each_width+go2_scale] = 2.1
                height[3*each_width:3*each_width+go2_scale] = .7

                # stair
                for i in range(14): # 0.3[m] * 14 = 4.2[m]
                    height[2*each_width-(i+1)*stair_step_height_map:2*each_width-i*stair_step_height_map, each_height:2*each_height] = 2.25 + .15 * i # 2.25 = 2.1 + 0.15
                    height[3*each_width-(i+1)*stair_step_height_map:3*each_width-i*stair_step_height_map, each_height:2*each_height] = .8 + .1 * i # 0.8 = 0.7 + 0.1
                    height[4*each_width-(i+1)*stair_step_height_map:4*each_width-i*stair_step_height_map, each_height:2*each_height] = .05 * (i+1)
                
                # boundary
                # for i in range(each_width):
                #     height[2*each_width-i-1, [0, -1]] = 2.1 * (1 + (i+1)/each_width)
                #     height[3*each_width-i-1, [0, -1]] = .7 + 1.4 * (i+1)/each_width
                #     height[4*each_width-i-1, [0, -1]] = .7*(i+1)/each_width

                # slope
                # for i in range(each_width):
                #     height[2*each_width-i-1, :each_height] = 2.1 * (1 + (i+1)/each_width)
                #     height[3*each_width-i-1, :each_height] = .7 + 1.4 * (i+1)/each_width
                #     height[4*each_width-i-1, :each_height] = .7*(i+1)/each_width
                
                for i in range(each_width-go2_scale):
                    height[2*each_width-i-1, :each_height] = 2.1 * (1 + (i+1)/(each_width-go2_scale))
                    height[3*each_width-i-1, :each_height] = .7 + 1.4 * (i+1)/(each_width-go2_scale)
                    height[4*each_width-i-1, :each_height] = .7*(i+1)/(each_width-go2_scale)
                ...
                # plane, center plane
                self.spawn_points = {# "plane": (5*each_width, each_half_height), 
                                     "plane": (5*each_width, 5*each_half_height), 
                                     "plane_center": (5*each_width + each_half_width, 3*each_half_height),
                                    #  "rough": (each_half_width, each_half_height), 
                                    #  "rough_center": (each_half_width, each_half_height),
                                     "rough": (each_half_width, 5*each_half_height), 
                                     "rough_center": (each_half_width, 5*each_half_height),
                                     "stair_5cm_center": (4*each_width + each_half_width, 3*each_half_height),
                                     "stair_10cm_center": (3*each_width + each_half_width, 3*each_half_height),
                                     "stair_15cm_center": (2*each_width + each_half_width, 3*each_half_height),
                                    #  "slope_0-5cm": (4*each_width + half_go2_scale, each_half_height),
                                    ...
                                    #  "slope_10-15cm": (2*each_width + half_go2_scale, each_half_height),
                                    #  "slope_15-cm": (each_width + half_go2_scale, each_half_height),
                                     "slope_0-5cm": (4*each_width + half_go2_scale, 5*each_half_height),
                                     "slope_5-10cm": (3*each_width + half_go2_scale, 5*each_half_height),
                                     "slope_10-15cm": (2*each_width + half_go2_scale, 5*each_half_height),
                                     "slope_15-cm": (each_width + half_go2_scale, 5*each_half_height),
                                     "stair_0-5cm": (4*each_width + half_go2_scale, 3*each_half_height),
                                     "stair_5-10cm": (3*each_width + half_go2_scale, 3*each_half_height),
                                     "stair_10-15cm": (2*each_width + half_go2_scale, 3*each_half_height),
                                     "stair_15-cm": (each_width + half_go2_scale, 3*each_half_height),
        ...
        self.task_dict = self.generate_task()
        self.chosen_task = [random.choice(list(self.task_dict.keys())) for _ in range(self.num_envs)]
        # print("chosen task: ", self.chosen_task)
        # self.update_init_spawn() # generate base_init pos, quat <- write reset_idx.

        ### change init pos and quat
        ...
        if self.terrain_type != "plane" and self.terrain_type != "custom_plane":
            ...
            terrain_origin_x, terrain_origin_y, terrain_origin_z = -self.center_x, -self.center_y, 0
            # self.height_field = self.global_terrain.geoms[0].metadata["height_field"]
            print("height field: ", self.height_field)
            for row in range(self.rows):
                for col in range(self.cols):
                    ...
                    # subterrain_center_z = (self.height_field[int(subterrain_center_x), int(subterrain_center_y)] ) * self.vertical_scale 
        ...
        self.hip_indices = find_link_indices(
            self.env_cfg['hip_link_name']
        )
    ...
    def generate_task(self):
        curr_level = self.curriculum_level_flag
        task_dict = {}

        for level, d in self.curriculum_tasks.items():
            if level <= curr_level:
                task_dict.update(d)
    ...
    def init_buffers(self):
        ...
        self.phase = torch.zeros((self.num_envs,), device=self.device, dtype=gs.tc_float)
        ...
        self.feet_pos_last_contacts = torch.zeros(
            (self.num_envs, len(self.feet_indices), 3),
            device=self.device,
            dtype=gs.tc_float,
        )

        self.step_distance = torch.zeros(
            (self.num_envs, len(self.feet_indices), 3),
            device=self.device,
            dtype=gs.tc_float,
        )

        self.cumulative_commands = torch.zeros(
            (self.num_envs, len(self.feet_indices), 2), # this 2 is x, y - coodinates.
            device=self.device,
            dtype=gs.tc_float,
        )

        self.elapsed_air_time = torch.zeros(
        ...
        self.projected_gravity_history = torch.zeros((self.num_envs, self.history_length, 3), device=self.device, dtype=gs.tc_float)
        ...
        self.hip_dof_pos = torch.zeros_like(self.hip_actions)
        self.hip_dof_vel = torch.zeros_like(self.hip_actions)
        ...
        self.contact_forces_history = torch.zeros(
            (self.num_envs, self.history_length, self.robot.n_links, 3), device=self.device, dtype=gs.tc_float
        )
        self.contact_forces_rate_history = torch.zeros(
            (self.num_envs, self.history_length, self.robot.n_links, 3), device=self.device, dtype=gs.tc_float
        )
        # self.contact_forces_smoothness_history = torch.zeros(
        #     (self.num_envs, self.history_length, self.robot.n_links, 3), device=self.device, dtype=gs.tc_float
        # )
        ...
        self.disable_actions_counter = torch.zeros((self.num_envs, self.num_actions), device=self.device, dtype=gs.tc_float)
        self.default_actions = self.default_dof_pos.unsqueeze(0).repeat(self.num_envs, 1)
    ...
    def _resample_commands_according_to_tasks(self, envs_idx):
        if self.curriculum:
            idx = envs_idx.tolist()
            tasks = [self.chosen_task[i] for i in idx]
            yaw = quat_to_xyz(self.base_quat[envs_idx])[:, 2]

            # [stand] or [otherwise]
            self.commands[envs_idx, :] = 0.

            # check [stand]
            # mask_stand = torch.tensor([ "stand" in t for t in tasks ])
            # print("mask stand sum: ", mask_stand.sum())

            # [walk_linear]
            mask_linear = torch.tensor([ "walk_linear" in t for t in tasks ])
            # print("mask linear sum: ", mask_linear.sum())
            if mask_linear.any():
                # # left spawn
                # self.commands[envs_idx[mask_linear], 0] = torch.sin(yaw[mask_linear]) # cos(pi/2 - yaw)
                # self.commands[envs_idx[mask_linear], 1] = torch.cos(yaw[mask_linear]) # sin(pi/2 - yaw)
                # right spawn
                self.commands[envs_idx[mask_linear], 0] = -torch.sin(yaw[mask_linear]) # -cos(pi/2 - yaw)
                self.commands[envs_idx[mask_linear], 1] = -torch.cos(yaw[mask_linear]) # -sin(pi/2 - yaw)
                scale = gs_rand_float(0.0, self.command_cfg["lin_vel_x_range"][1], (mask_linear.sum(),1), self.device)
                self.commands[envs_idx[mask_linear], :2] *= scale

            # [walk_angular]
            mask_angular = torch.tensor([ "walk_angular" in t for t in tasks ])
            # print("mask angular sum: ", mask_angular.sum())
            if mask_angular.any():
                self.commands[envs_idx[mask_angular], 0] = torch.sin(yaw[mask_angular]) # cos(pi/2 - yaw)
                self.commands[envs_idx[mask_angular], 1] = torch.cos(yaw[mask_angular]) # sin(pi/2 - yaw)
                scale = gs_rand_float(0.0, self.command_cfg["lin_vel_x_range"][1], (mask_angular.sum(),1), self.device)
                self.commands[envs_idx[mask_angular], :2] *= scale
                self.commands[envs_idx[mask_angular], 2] = gs_rand_float(*self.command_cfg["ang_vel_range"], (mask_angular.sum(),), self.device)

            # [walk_up]
            mask_up = torch.tensor([ "walk_up" in t for t in tasks ])
            if mask_up.any():
                self.commands[envs_idx[mask_up], 0] = -torch.cos(yaw[mask_up]) # cos(pi - yaw)
                self.commands[envs_idx[mask_up], 1] = torch.sin(yaw[mask_up]) # sin(pi - yaw)
                scale = gs_rand_float(self.command_cfg["lin_vel_x_range"][1]/2, self.command_cfg["lin_vel_x_range"][1], (mask_up.sum(),1), self.device)
                self.commands[envs_idx[mask_up], :2] *= scale

            # [walk_down]
            mask_down = torch.tensor([ "walk_down" in t for t in tasks ])
            if mask_down.any():
                self.commands[envs_idx[mask_down], 0] = torch.cos(yaw[mask_down]) # cos(-yaw)
                self.commands[envs_idx[mask_down], 1] = -torch.sin(yaw[mask_down]) # sin(-yaw)
                scale = gs_rand_float(self.command_cfg["lin_vel_x_range"][1]/2, self.command_cfg["lin_vel_x_range"][1], (mask_down.sum(),1), self.device)
                self.commands[envs_idx[mask_down], :2] *= scale
    ...
    def init_foot(self):
        ...
        # self.thigh_vel = all_links_vel[:, self.thigh_indices, :]
        ...
        self.feet_pos_base = self._world_to_base_transform(self.feet_pos, self.base_pos, self.base_quat)
        ...
        self.robot.set_friction_ratio(
            friction_ratio=torch.ones(self.num_envs, self.robot.n_links, device=self.device)*1.,
            links_idx_local=np.arange(0, self.robot.n_links),
            envs_idx=np.arange(0, self.num_envs),
        )
    ...
    def update_feet_state(self):
        ...
        self.feet_pos = all_links_pos[:, self.feet_indices, :]
        print("feet pos-z: ", self.feet_pos[0, :, 2])
        self.feet_vel = all_links_vel[:, self.feet_indices, :]
        # self.hip_pos = all_links_pos[:, self.hip_indices, :]
        # self.hip_vel = all_links_vel[:, self.hip_indices, :]
        self.thigh_pos = all_links_pos[:, self.thigh_indices, :]
    ...
    def post_physics_step_callback(self):
        ...
        commands_xy = torch.norm(self.commands[:, :2], dim=1)
        # print("commands_xy: ", commands_xy.shape)
        self.phase += (commands_xy * self.dt) % self.step_period / self.step_period
        self.phase %= 1
        # print("phase: ", self.phase[0])
        ...
        # print("self.leg_phase: ", self.leg_phase.shape)

    def step(self, actions):
        # print("stand", sum(["stand" in t for t in self.chosen_task]))
        # print("walk_linear", sum(["walk_linear" in t for t in self.chosen_task]))
        # print("walk_angular", sum(["walk_angular" in t for t in self.chosen_task]))
        # print("walk_up", sum(["walk_up" in t for t in self.chosen_task]))
        # print("walk_down", sum(["walk_down" in t for t in self.chosen_task]))
        # if self.episode_length_buf >0:
        self.actions = torch.clip(actions, -self.env_cfg["clip_actions"], self.env_cfg["clip_actions"])

        # print(self.robot.material)
        # print(self.robot.material.restitution)

        # height = self.get_terrain_height_at(self.feet_pos[:, :, 0], self.feet_pos[:, :, 1])
        # is_falling = (self.feet_pos[:, :, 2] - height) > 0.01
        # is_any_falling = torch.any(is_falling, dim=1)
        # is_all_falling = torch.all(is_falling, dim=1)
        # self.actions[is_all_falling] = self.default_actions[is_all_falling]

        mask_action = self.disable_actions_counter > 0
        self.actions[mask_action] = self.default_actions[mask_action]
        mask_action_ids = torch.arange(0, self.num_envs)[torch.any(mask_action, dim=1)]
        self.robot.set_pos(self.base_pos[mask_action_ids, :], zero_velocity=True, envs_idx=mask_action_ids)
        self.robot.set_quat(self.base_quat[mask_action_ids, :], zero_velocity=True, envs_idx=mask_action_ids)
        self.disable_actions_counter[mask_action] -= 1
        ...
        # exec_actions = torch.nan_to_num(exec_actions, nan=0.0, posinf=self.clip_obs, neginf=-self.clip_obs)
        ...
        print("base_pos: ", self.base_pos[0, :])
        ...
        print("base_lin_vel_z: ", self.base_lin_vel[0, 2])
        self.base_ang_vel[:] = transform_by_quat(self.robot.get_ang(), inv_base_quat)
        self.projected_gravity = transform_by_quat(self.global_gravity, inv_base_quat)
        self.dof_pos[:] = self.robot.get_dofs_position(self.motor_dofs)
        self.dof_vel[:] = self.robot.get_dofs_velocity(self.motor_dofs)
        self.hip_dof_pos[:] = self.robot.get_dofs_position(self.hip_dofs)
        self.hip_dof_vel[:] = self.robot.get_dofs_velocity(self.hip_dofs)
        ...
        self.projected_gravity_history[:, :-1, :] = self.projected_gravity_history[:, 1:, :].clone()
        self.projected_gravity_history[:, -1, :] = self.projected_gravity

        self.contact_forces_history[:, :-1, :, :] = self.contact_forces_history[:, 1:, :, :].clone()
        self.contact_forces_history[:, -1, :, :] = self.contact_forces

        self.contact_forces_rate_history[:, :-1, :, :] = self.contact_forces_rate_history[:, 1:, :, :].clone()
        self.contact_forces_rate_history[:, -1, :, :] = \
            (self.contact_forces_history[:, -1, :, :] - self.contact_forces_history[:, -2, :, :])*self.dt
        ...
        if self.curriculum and not self.curriculum_complete_flag:
        ...
        # elif not self.control_:
        #     self._resample_commands_according_to_tasks(envs_idx)

        
        if not self.control_:
            self._resample_commands_according_to_tasks(envs_idx)
        ...
        first_contact = torch.logical_and(contact, ~self.last_contacts)
        last_contact = torch.logical_and(~contact, self.last_contacts)
        commands = self.commands*self.commands_scale

        self.feet_pos_last_contacts[last_contact] = self.feet_pos_base[last_contact]
        self.step_distance[~first_contact] = 0
        self.step_distance[first_contact] = self.feet_pos_base[first_contact] - self.feet_pos_last_contacts[first_contact]
        self.elapsed_air_time[~contact] += self.dt

        cos_yaw, sin_yaw = torch.cos(commands[:,2]*self.dt), torch.sin(commands[:,2]*self.dt)
        R = torch.stack([
            torch.stack([cos_yaw, -sin_yaw], dim=-1),
            torch.stack([sin_yaw,  cos_yaw], dim=-1)
            ], dim=1)
        
        step_commands = commands[:, :2].unsqueeze(1)*self.dt
        rotated = torch.einsum("nij,nfj->nfi", R, self.cumulative_commands)

        self.cumulative_commands = step_commands + rotated
        ...
        # if len(envs_idx):
        #     print("pos_sample:", self.base_pos[envs_idx[0]])
        #     # after you set base_quat[...] and before set_quat(...)
        #     print("quat_sample:", self.base_quat[envs_idx[0]])
        #     # convert one quaternion to rpy and print (helper below)
        #     q = self.base_quat[envs_idx[0]]
        #     print("rpy(deg):", quaternion_to_euler_deg(q))

        self.reset_idx(self.reset_buf.nonzero(as_tuple=False).flatten())
        self._render_headless()
        self.extras["observations"]["critic"] = self.privileged_obs_buf

        print("curriculum level: ", self.curriculum_level_flag)
    ...
    def compute_observations(self):
        ...
        forces_rate_xy_feet = torch.max(torch.norm(self.contact_forces_rate_history[:, :-1, self.feet_indices,:2],dim=3),dim=1).values
        forces_rate_xy_calf = torch.max(torch.norm(self.contact_forces_rate_history[:, :-1, self.calf_indices,:2],dim=3),dim=1).values
        # print("forces_rate_xy_feet: ", forces_rate_xy_feet)
        # Debug checks
        debug_items = {
        ...
        component_data_dict = {
            ...
            "cos_phase": cos_phase,
            "collision": collision,
            "forces_rate_xy_feet": forces_rate_xy_feet,
            "forces_rate_xy_calf": forces_rate_xy_calf,
        }
        ...
        self.obs_buf = torch.nan_to_num(self.obs_buf, nan=0.0, posinf=self.clip_obs, neginf=-self.clip_obs)
        self.privileged_obs_buf = torch.nan_to_num(self.privileged_obs_buf, nan=0.0, posinf=self.clip_obs, neginf=-self.clip_obs)
        ...
        self.elapsed_air_time[first_contact] = 0
    ...
    def compute_rewards(self):
        ...
        for name, reward_func in self.reward_functions.items():
            ...
            # if name == "step_distance":
            #     x = rew
            # elif name == "step_distance_penalty":
            #     y = rew
            self.rew_buf += rew
        ...
        # # 共分散を計算
        # cov_xy = (x_mean * y_mean).mean()  # 不偏の場合は / (len(x)-1)
        # print(cov_xy)
    ...
    def reset_idx(self, envs_idx):
        ...
        # disable action counter of respawn
        self.disable_actions_counter[envs_idx] = 5 #10
        ...
        if self.curriculum:
            if (self.mean_reward > self.mean_reward_threshold) and (self.curriculum_level_flag < max(self.curriculum_tasks.keys())):
                self.curriculum_level_flag += 1
                self.task_dict = self.generate_task()
                self.chosen_task = [random.choice(list(self.task_dict.keys())) for _ in range(self.num_envs)]
                self.mean_reward_threshold += self.command_cfg["mean_reward_threshold"]/2
                if self.curriculum_level_flag > 1:
                    self.reward_cfg["reward_scales"]["orientation"] = -.01
                    self.reward_cfg["reward_scales"]["ang_vel_xy"] = -0.05
                if self.curriculum_level_flag == 1:
                    # self.reward_cfg["tracking_min_sigma"] /= 2
                    self.reward_cfg["reward_scales"]["lin_vel_z"] = -3.
                    self.reward_cfg["reward_scales"]["stand_pos"] = -0.1
                    self.reward_cfg["reward_scales"]["contact_stair"] = -1.
                if self.curriculum_level_flag == 2:
                    # self.reward_cfg["tracking_min_sigma"] *= 2
                    self.reward_cfg["reward_scales"]["lin_vel_z"] = -2.
                    self.reward_cfg["reward_scales"]["stand_pos"] = -0.05
                    self.termination_if_roll_greater_than_value = 90
                if self.curriculum_level_flag == 3:
                    # self.reward_cfg["tracking_min_sigma"] /= 2
                    self.termination_if_roll_greater_than_value = 120 # 170
                    # self.termination_if_pitch_greater_than_value = 170

            self.reset_init_spawn(envs_idx)
        ...
        # reset buffers
        self.projected_gravity_history[envs_idx] = 0.0
        self.contact_forces_history[envs_idx] = 0.0
        self.contact_forces_rate_history[envs_idx] = 0.0
        ...
        self.hip_dof_pos[envs_idx] = 0.0
        self.hip_dof_vel[envs_idx] = 0.0
        ...
        self.last_contacts[envs_idx]          = 0.0
        self.feet_pos_last_contacts[envs_idx] = 0.0
        self.step_distance[envs_idx]          = 0.0
        self.cumulative_commands[envs_idx]    = 0.0
        self.elapsed_air_time[envs_idx]       = 0.0
    ...
    def reset_init_spawn(self, env_ids=None):
        for i in env_ids:
            # self.probability_to_change_tasks
            task = self.chosen_task[i]
            spawn_name = self.task_dict[task]
            
            # terrain's map coodinates
            x, y = self.spawn_points[spawn_name]
            z = self.height_field[x, y]

            # ワールド座標に変換
            world_x = x * self.horizontal_scale - self.center_x + self.base_init_pos[0]
            world_y = y * self.horizontal_scale - self.center_y + self.base_init_pos[1]
            world_z = z * self.vertical_scale + self.base_init_pos[2]

            # if "-" in task:
            #     world_z += 0.05

            # その env だけ更新
            self.base_pos[i] = torch.tensor([world_x, world_y, world_z], device=self.device)
        ...
        if self.env_cfg["randomize_rot"]:
            ...
            # roll = gs_rand_float(*self.env_cfg["roll_range"],  (len(env_ids),), self.device)
            # pitch = gs_rand_float(*self.env_cfg["pitch_range"], (len(env_ids),), self.device)
            yaw = gs_rand_float(*self.env_cfg["yaw_range"], (len(env_ids),), self.device)

            idx = env_ids.tolist()
            tasks = [self.chosen_task[i] for i in idx]
            mask_5cm = torch.tensor([ "5cm_center" in t for t in tasks ])
            pitch[mask_5cm] = math.degrees(math.atan2(0.3, 0.05))
            mask_10cm = torch.tensor([ "10cm_center" in t for t in tasks ])
            pitch[mask_10cm] = math.degrees(math.atan2(0.3, 0.1))
            mask_15cm = torch.tensor([ "15cm_center" in t for t in tasks ])
            pitch[mask_15cm] = math.degrees(math.atan2(0.3, 0.15))

            # 2) Convert them all at once into a (N,4) quaternion tensor [w, x, y, z].
            quats_torch = quaternion_from_euler_tensor(roll, pitch, yaw)  # (N, 4)
            # 3) Move to CPU if needed and assign into self.base_quat in one shot
            self.base_quat[env_ids] = quats_torch
        else:
    ...
    def _reward_tracking_lin_vel(self):
        ...
        # cmd_vel_square = torch.clamp(torch.norm(self.commands[:, :2], dim=1), min=self.reward_cfg["tracking_min_sigma"], max=self.reward_cfg["tracking_max_sigma"])
        cmd_vel_square = torch.clamp(torch.sum(torch.square(self.commands[:, :2]), dim=1), min=self.reward_cfg["tracking_min_sigma"], max=self.reward_cfg["tracking_max_sigma"])
        return torch.exp(-lin_vel_error / cmd_vel_square)

    def _reward_tracking_ang_vel(self):
        # Tracking of angular velocity commands (yaw)
        ang_vel_error = torch.square(self.commands[:, 2] - self.base_ang_vel[:, 2])
        # return torch.exp(-ang_vel_error / self.reward_cfg["tracking_sigma"])
        cmd_vel_square = torch.clamp(torch.square(self.commands[:, 2]), min=self.reward_cfg["tracking_min_sigma"], max=self.reward_cfg["tracking_max_sigma"])
        return torch.exp(-ang_vel_error / cmd_vel_square)
    ...
    def _reward_relative_base_height(self):
        ...
        pitch_ok = (torch.abs(self.base_euler[:, 1]) < math.radians(5.0)) # Bool tensor shape = (N,)
    ...
    def _reward_hip_pos(self):
        return torch.sum(torch.abs(self.hip_dof_pos- self.default_hip_pos), dim=(1))
    ...
    def _reward_front_feet_clearance(self):
        ...
        max_bonus = (self.reward_cfg["max_reward_height"] - self.reward_cfg["clearance_start"]) ** 2

        above_target = torch.clamp(rel_h - self.reward_cfg["clearance_start"], min=0.0)
    ...
    def _reward_hip_dof_vel(self):
        # Penalize dof velocities
        return torch.sum(torch.square(self.hip_dof_vel), dim=1)

    def _reward_thigh_dof_vel(self):
        # Penalize dof velocities
        return torch.sum(torch.square(self.thigh_dof_vel), dim=1)
    ...
    def _reward_feet_air_time(self):
        ...
        rew_airTime = torch.sum((self.feet_air_time - self.step_period / 2) * first_contact, dim=1) # reward only on first contact with the ground
    ...
    def _reward_foot_vel(self):
        cmd_weight = torch.exp(-(torch.norm(self.commands, dim=1)/0.1) **2)
        foot_vel_term = torch.sum(torch.norm(self.feet_vel, dim=2), dim=1)
        penalty = cmd_weight * foot_vel_term
        return penalty
    
    def _reward_contact_no_commands(self):
        cmd_weight = torch.exp(-(torch.norm(self.commands, dim=1)/0.1) **2)
        contact_term = torch.sum(torch.exp(-torch.sum(torch.square(self.contact_forces[:, self.feet_indices]), dim=2)), dim=1)
        penalty = cmd_weight * contact_term
        return penalty
    
    def _reward_stand_pos(self):
        cmd_weight = torch.exp(-(torch.norm(self.commands, dim=1)/0.1) **2)
        default_pos_error = torch.sum(torch.abs(self.dof_pos - self.default_dof_pos), dim=1)
        penalty = cmd_weight * default_pos_error
        return penalty

    def _reward_contact_stair(self):
        threshold_forces_rate = 1.
        contact_xy_feet = torch.any((torch.norm(self.contact_forces_rate_history[:, :-1, self.feet_indices,:2],dim=3) > threshold_forces_rate),dim=1)
        contact_xy_calf = torch.any((torch.norm(self.contact_forces_rate_history[:, :-1, self.calf_indices,:2],dim=3) > threshold_forces_rate),dim=1)

        height = self.get_terrain_height_at(self.feet_pos[:, :, 0], self.feet_pos[:, :, 1])
        # print("feet_pos z shape:", self.feet_pos[:, :, 2].shape)
        # print("height shape:", height.shape)
        # print("contact_xy shape:", contact_xy.shape)
        feet_target_error = torch.sqrt((self.feet_pos[:, :, 2] - height - 0.15)**2 + 1e-6)
        penalty = torch.sum(feet_target_error * contact_xy_feet, dim=1)
        penalty += torch.sum(feet_target_error * contact_xy_calf, dim=1)

        # contact_xy_any = contact_xy_feet | contact_xy_calf
        # penalty[~torch.any(contact_xy_any, dim=1)] = 1
    ...
    def _reward_both_front_feet_airborne(self):
        ...
        # 1. 前足（左右）のコンタクト状態を取得
        contact = torch.norm(
            self.contact_forces[:, self.feet_front_indices, :], dim=2
        ) > 1.0  # shape: (N, 2)
    ...
    def _reward_both_rear_feet_airborne(self):
        ...
        # 1. 前足（左右）のコンタクト状態を取得
        contact = torch.norm(
            self.contact_forces[:, self.feet_rear_indices, :], dim=2
        ) > 1.0  # shape: (N, 2)
    ...
    # def _reward_hip_thigh_vel(self):
    #     """
    #     hipやthighの動きを抑えるための報酬
    #     """
    #     hip_abs_vel = torch.abs(self.hip_dof_vel * self.obs_scales["dof_vel_scaled"])
    #     thigh_abs_vel = torch.abs(self.thigh_dof_vel * self.obs_scales["dof_vel_scaled"])
    #     commands = self.commands
    #     cmd_x_yaw_abs_vel = torch.abs(commands[:,0])+torch.abs(commands[:,2])
    #     cmd_y_yaw_abs_vel = torch.abs(commands[:,1]+commands[:,2])
        
    #     penalty = torch.abs(torch.sum(hip_abs_vel, dim=1)*2 - cmd_y_yaw_abs_vel)
    #     penalty += torch.abs(torch.sum(thigh_abs_vel, dim=1)*2 - cmd_x_yaw_abs_vel)

    #     return penalty

    def _reward_step_distance(self):
        norm_step_distance = torch.norm(self.step_distance, dim=(1,2))

        # norm_step_distance = torch.zeros_like(norm_step_distance) # debug
        return norm_step_distance
    
    def _reward_front_step_distance(self):
        """（四足用）"""
        norm_step_distance = torch.norm(self.step_distance[:, :2], dim=(1,2))

        # norm_step_distance = torch.zeros_like(norm_step_distance) # debug
        return norm_step_distance
    
    def _reward_rear_step_distance(self):
        """（四足用）"""
        norm_step_distance = torch.norm(self.step_distance[:, 2:], dim=(1,2))
    ...
    def _reward_front_step_distance_commmands(self):
        """
        累計のコマンドに合わせた前足の動きができているか？（四足用）
        """
        inner_mul_dist_commands = (self.step_distance[:,:2,:2]*self.cumulative_commands[:,:2,:]).sum(dim=(1,2))
        # norm_step_distance = torch.zeros_like(norm_step_distance) # debug
        return inner_mul_dist_commands
    
    def _reward_rear_step_distance_commmands(self):
        """
        累計のコマンドに合わせた後ろ足の動きができているか？（四足用）
        """
        inner_mul_dist_commands = (self.step_distance[:,2:,:2]*self.cumulative_commands[:,2:,:]).sum(dim=(1,2))
        # norm_step_distance = torch.zeros_like(norm_step_distance) # debug
        return inner_mul_dist_commands

    def _reward_step_distance_penalty(self):
        contact = torch.norm(self.contact_forces[:, self.feet_indices, :], dim=2) > 1.
        first_contact = torch.logical_and(contact, ~self.last_contacts)

        square_step_distance = torch.exp(-torch.square(self.step_distance))
        square_step_distance[~first_contact] = 0 # 接地したタイミングのみペナルティを与えたい
        penalty = torch.sum(square_step_distance, dim=(1,2))

        return penalty

    def _reward_step_timing(self):
        contact = torch.norm(self.contact_forces[:, self.feet_indices, :], dim=2) > 1.
        first_contact = torch.logical_and(contact, ~self.last_contacts)

        target_interval = self.reward_cfg["step_period"]/2
        gait_timing = torch.square(self.elapsed_air_time - target_interval)

        gait_timing[~first_contact] = 0
        penalty = torch.sum(gait_timing,dim=1)

        return penalty
    
    def _reward_front_step_timing(self):
        contact = torch.norm(self.contact_forces[:, self.feet_indices, :], dim=2) > 1.
        first_contact = torch.logical_and(contact, ~self.last_contacts)
        ...
        gait_timing[~first_contact] = 0
        penalty = torch.sum(gait_timing[:, :2],dim=1)

        return penalty
    
    def _reward_rear_step_timing(self):
        contact = torch.norm(self.contact_forces[:, self.feet_indices, :], dim=2) > 1.
        first_contact = torch.logical_and(contact, ~self.last_contacts)
