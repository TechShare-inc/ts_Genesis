...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="Z_walking")
    ...
    # env_cfg["base_init_quat"] = [0.70710678, 0.0, 0.0, 0.70710678]
    # env_cfg["base_init_quat"] = [0., 0., 0., 1.]
    ...
    if EXPORT_POLICY:
        ...
        if class_name == "ActorCritic":
        ...
        elif class_name == "ActorCriticRecurrent" and train_cfg["policy"]["rnn_type"] == "lstm":
            ...
            versionless_path = os.path.join(save_dir, "actor_lstm_safe.pt")
            loaded_model = torch.jit.load(save_path)
            loaded_model.eval()
            loaded_model.save(versionless_path)
            print("Model successfully converted to version-less? format: ", versionless_path)
...
"""
# evaluation
python legged_eval.py -e Z_walking --ckpt 100
"""
