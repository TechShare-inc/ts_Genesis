        ...
        if morph.height_field is not None:
            ...
            for i in range(morph.n_subterrains[0]):
                for j in range(morph.n_subterrains[1]):
                    ...
                    elif subterrain_type == "variable_terrain" and morph.n_subterrains[0] == morph.n_subterrains[1] == 1:
                        new_subterrain.vertical_scale = 0.01
                        new_subterrain.horizontal_scale = 1/256 # 256
                        new_subterrain.width = int(40/new_subterrain.horizontal_scale)
                        new_subterrain.length = int(16/new_subterrain.horizontal_scale)
                        subterrain_height_field = variable_set_terrain(
                            new_subterrain,
                        ).height_field_raw
                    elif subterrain_type == "variable_terrain2" and morph.n_subterrains[0] == morph.n_subterrains[1] == 1:
                        new_subterrain.vertical_scale = .005
                        new_subterrain.horizontal_scale = .01 # 1/100
                        new_subterrain.width = int(36.4/new_subterrain.horizontal_scale)
                        new_subterrain.length = int(15.6/new_subterrain.horizontal_scale)
                        subterrain_height_field = variable_set_terrain2(
                            new_subterrain,
                        ).height_field_raw
                    elif subterrain_type == "variable_terrain3" and morph.n_subterrains[0] == morph.n_subterrains[1] == 1:
                        new_subterrain.vertical_scale = .0025
                        new_subterrain.horizontal_scale = .02 # 1/100
                        new_subterrain.width = int(35./new_subterrain.horizontal_scale)
                        new_subterrain.length = int(15./new_subterrain.horizontal_scale)
                        subterrain_height_field = variable_set_terrain3(
                            new_subterrain,
                        ).height_field_raw
                    else:
...
def variable_set_terrain(terrain):
    width = terrain.width
    length = terrain.length
    sub_terrains = (20, 8)
    each_width = int(width/sub_terrains[0])
    each_length = int(length/sub_terrains[1])

    stair_step_height_map = int(0.3/terrain.horizontal_scale) # 0.3 = a step of stair

    # boundary_width_list = [0, each_width-1, each_width, 4*each_width-1, 4*each_width, 5*each_width-1, 5*each_width,
    #               8*each_width-1, 16*each_width, -each_width-1, -each_width]
    # boundary_length_list = [0, 2*each_length-1, -2*each_length, -1]

    height = np.zeros((width, length))

    # plane
    height[:each_width] = 3.0
    height[4*each_width:5*each_width] = 1.0
    height[-each_width:] = 3.0
    
    # stair
    for i in range(20): # 0.3[m] * 20 = 6[m]
        height[each_width+i*stair_step_height_map:each_width+(i+1)*stair_step_height_map][2*each_length:6*each_length] = 2.9 - .1 * i
        height[5*each_width+i*stair_step_height_map:5*each_width+(i+1)*stair_step_height_map][2*each_length:6*each_length] = .9 - .05 * i
        height[18*each_width+i*stair_step_height_map:18*each_width+(i+1)*stair_step_height_map][2*each_length:6*each_length] = .15 * i
    
    # boundary
    for i in range(3*each_width):
        height[each_width+i][[0, -1]] = 3.0 - 2.0 * (i+1)/(3*each_width)
        height[4*each_width+i][[0, -1]] = 1.0 - 1.0 * (i+1)/(3*each_width)
        height[-4*each_width+i][[0, -1]] = 3.0 * i/(3*each_width)

    # slope
    for j in range(1, 2*each_length):
        height[each_width:4*each_width][j] = (height[each_width:4*each_width][0]*(2*each_length-j) + height[each_width:4*each_width][2*each_length]*j)/each_length
        height[5*each_width:8*each_width][j] = (height[5*each_width:8*each_width][0]*(2*each_length-j) + height[5*each_width:8*each_width][2*each_length]*j)/each_length
        height[-4*each_width:-each_width][j] = (height[-4*each_width:-each_width][0]*(2*each_length-j) + height[-4*each_width:-each_width][2*each_length]*j)/each_length

        height[each_width:4*each_width][-j-1] = height[each_width:4*each_width][j]
        height[5*each_width:8*each_width][-j-1] = height[5*each_width:8*each_width][j]
        height[-4*each_width:-each_width][-j-1] = height[-4*each_width:-each_width][j]
...
def variable_set_terrain2(terrain):
    width = terrain.width
    length = terrain.length
    sub_terrains = (7, 3)
    each_width = int(width/sub_terrains[0])
    ...
    stair_step_height_map = int(0.3/terrain.horizontal_scale) # 0.3 = a step of stair
    go2_scale = int(0.8/terrain.horizontal_scale) # go2's xy-length(L2-norm)
    ...
    for i in range(6): # 0.3[m] * 6 = 1.8[m]
        height[2*each_width-(i+1)*stair_step_height_map:2*each_width-i*stair_step_height_map, each_length:2*each_length] = 1.05 + .15 * i # 1.05 = 0.9 + 0.15
        height[3*each_width-(i+1)*stair_step_height_map:3*each_width-i*stair_step_height_map, each_length:2*each_length] = .4 + .1 * i # 0.4 = 0.3 + 0.1
        height[4*each_width-(i+1)*stair_step_height_map:4*each_width-i*stair_step_height_map, each_length:2*each_length] = .05 * (i+1)
    ...
    # slope
    for j in range(1, each_length):
        # height[each_width+go2_scale:2*each_width, j] = (height[each_width+go2_scale:2*each_width, 0]*(each_length-j) + height[each_width+go2_scale:2*each_width, each_length]*j)/each_length
        # height[2*each_width+go2_scale:3*each_width, j] = (height[2*each_width+go2_scale:3*each_width, 0]*(each_length-j) + height[2*each_width+go2_scale:3*each_width, each_length]*j)/each_length
        # height[3*each_width+go2_scale:4*each_width, j] = (height[3*each_width+go2_scale:4*each_width, 0]*(each_length-j) + height[3*each_width+go2_scale:4*each_width, each_length]*j)/each_length

        height[each_width:2*each_width, j] = (height[each_width:2*each_width, 0]*(each_length-j) + height[each_width:2*each_width, each_length]*j)/each_length
        height[2*each_width:3*each_width, j] = (height[2*each_width:3*each_width, 0]*(each_length-j) + height[2*each_width:3*each_width, each_length]*j)/each_length
        height[3*each_width:4*each_width, j] = (height[3*each_width:4*each_width, 0]*(each_length-j) + height[3*each_width:4*each_width, each_length]*j)/each_length

        # height[each_width+go2_scale:2*each_width, -j-1] = height[each_width+go2_scale:2*each_width, j]
        # height[2*each_width+go2_scale:3*each_width, -j-1] = height[2*each_width+go2_scale:3*each_width, j]
        # height[3*each_width+go2_scale:4*each_width, -j-1] = height[3*each_width+go2_scale:4*each_width, j]
    height[:, -each_length:] = np.flip(height[:, :each_length], axis=1)
...
def variable_set_terrain3(terrain):
    width = terrain.width
    length = terrain.length
    sub_terrains = (7, 3)
    each_width = int(width/sub_terrains[0])
    ...
    for i in range(14): # 0.3[m] * 14 = 4.2[m]
        height[2*each_width-(i+1)*stair_step_height_map:2*each_width-i*stair_step_height_map, each_length:2*each_length] = 2.25 + .15 * i # 2.25 = 2.1 + 0.15
        height[3*each_width-(i+1)*stair_step_height_map:3*each_width-i*stair_step_height_map, each_length:2*each_length] = .8 + .1 * i # 0.8 = 0.7 + 0.1
        height[4*each_width-(i+1)*stair_step_height_map:4*each_width-i*stair_step_height_map, each_length:2*each_length] = .05 * (i+1)
    ...
    # slope
    # for i in range(each_width):
    #     height[2*each_width-i-1, :each_length] = 2.1 * (1 + (i+1)/each_width)
    #     height[3*each_width-i-1, :each_length] = .7 + 1.4 * (i+1)/each_width
    #     height[4*each_width-i-1, :each_length] = .7*(i+1)/each_width

    for i in range(each_width-go2_scale):
        height[2*each_width-i-1, :each_length] = 2.1 * (1 + (i+1)/(each_width-go2_scale))
        height[3*each_width-i-1, :each_length] = .7 + 1.4 * (i+1)/(each_width-go2_scale)
        height[4*each_width-i-1, :each_length] = .7*(i+1)/(each_width-go2_scale)
